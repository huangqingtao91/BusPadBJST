...
import com.ltime.buspad.bean.MusicData;
...
import com.ltime.buspad.service.MusicServicexx;
import com.ltime.buspad.util.Constants;
import com.ltime.buspad.util.DisplayUtil;
import com.ltime.buspad.util.FastBlurUtil;
import com.ltime.buspad.widget.BackgourndAnimationRelativeLayout;
import com.ltime.buspad.widget.DiscView;
...
import static com.ltime.buspad.widget.DiscView.DURATION_NEEDLE_ANIAMTOR;


public class MusicActivityA extends AppCompatActivity implements DiscView.IPlayInfo, View
        .OnClickListener {
    private ListView lv;
...
//        final List<AdResult> mlistad = new ArrayList<>();
        ...
        call.enqueue(new Callback<MusicResult>() {
            ...
            public void onResponse(Call<MusicResult> call, Response<MusicResult> response) {
                ...
                adapter = new ListMusicAdapter(MusicActivityA.this, mlist);
                lv.setAdapter(adapter);


                Intent intent = new Intent(MusicActivityA.this, MusicServicexx.class);
    ...
    private void initMusicReceiver() {
        ...
        intentFilter.addAction(MusicServicexx.ACTION_STATUS_MUSIC_PLAY);
        intentFilter.addAction(MusicServicexx.ACTION_STATUS_MUSIC_PAUSE);
        intentFilter.addAction(MusicServicexx.ACTION_STATUS_MUSIC_DURATION);
        intentFilter.addAction(MusicServicexx.ACTION_STATUS_MUSIC_COMPLETE);
        /*注册本地广播*/
    ...
    private Drawable getForegroundDrawable(int musicPicRes) {
        ...
        final float widthHeightSize = (float) (DisplayUtil.getScreenHeight(MusicActivityA.this)
                * 1.0 / DisplayUtil.getScreenWidth(this) * 1.0);
    ...
    private void play() {
        optMusic(MusicServicexx.ACTION_OPT_MUSIC_PLAY);
        startUpdateSeekBarProgress();
    }

    private void pause() {
        optMusic(MusicServicexx.ACTION_OPT_MUSIC_PAUSE);
        stopUpdateSeekBarProgree();
    }
    ...
    private void next() {
        mRootLayout.postDelayed(new Runnable() {
            @Override
            public void run() {
                optMusic(MusicServicexx.ACTION_OPT_MUSIC_NEXT);
            }
    ...
    private void last() {
        mRootLayout.postDelayed(new Runnable() {
            @Override
            public void run() {
                optMusic(MusicServicexx.ACTION_OPT_MUSIC_LAST);
            }
        }, DURATION_NEEDLE_ANIAMTOR);
        stopUpdateSeekBarProgree();
        mTvMusicDuration.setText(duration2Time(0));
    ...
    private void seekTo(int position) {
        Intent intent = new Intent(MusicServicexx.ACTION_OPT_MUSIC_SEEK_TO);
        intent.putExtra(MusicServicexx.PARAM_MUSIC_SEEK_TO,position);
    ...
    class MusicReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(MusicServicexx.ACTION_STATUS_MUSIC_PLAY)) {
                mIvPlayOrPause.setImageResource(R.mipmap.ic_pause);
                int currentPosition = intent.getIntExtra(MusicServicexx.PARAM_MUSIC_CURRENT_POSITION, 0);
                mSeekBar.setProgress(currentPosition);
                if(!mDisc.isPlaying()){
                    mDisc.playOrPause();
                }
            } else if (action.equals(MusicServicexx.ACTION_STATUS_MUSIC_PAUSE)) {
                mIvPlayOrPause.setImageResource(R.mipmap.ic_play);
                if (mDisc.isPlaying()) {
                    mDisc.playOrPause();
                }
            } else if (action.equals(MusicServicexx.ACTION_STATUS_MUSIC_DURATION)) {
                int duration = intent.getIntExtra(MusicServicexx.PARAM_MUSIC_DURATION, 0);
                updateMusicDurationInfo(duration);
            } else if (action.equals(MusicServicexx.ACTION_STATUS_MUSIC_COMPLETE)) {
                boolean isOver = intent.getBooleanExtra(MusicServicexx.PARAM_MUSIC_IS_OVER, true);
                complete(isOver);
            }
        }
    }
